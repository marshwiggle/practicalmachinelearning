...
The data are separated into two sets: the training set and the test set. They can be downloaded from the following URLs

```{r, echo=TRUE}
train_file_url = "https://d396qusza40orc.cloudfront.net/predmachlearn/pml-training.csv"
```

If we still have no local copy of the data file, now it's time to download it to our PC:
...
Let's load our datasets into R environment. As many columns contains invalid data, we set **as.is**
...
```
It was stated on the PML Course Forum, that these data allows to predict the result with 100% 
accuracy (for the cross-validation set), but in real life we 
can hardly get all this stuff, so it's better to ignore it completely (we will do it in a second). 
...
Now it is time to remove unnecessary columns, i.e. first 7 columns and columns that contains 
different types of summaries: mean,  variance, standard deviation, max, min, amplitude and kurtosis. These columns contain a lot of  "NAs" and it's rather hard to use them in real life.
...
It is mentions in  the paper of Velloso et all that all of them give considerably good results. 
In this paper we will compare only two methods: Gradient Boosting Machine and Random Forests. 

## Training

First, we split our data frame into feature columns and "prediction" column:
```{r, echo=TRUE}
  x <- training[,-53]
  y <- training[, 53]
```  
...
The main practical problem with training is that it takes a lot of time. To make it run faster,
it's good idea to use parallel computations. The following code is 
